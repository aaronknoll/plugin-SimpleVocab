...
class SimpleVocabPlugin
{
    public static function install()
    {
        ...
        $db->query($sql);
		 ...
		 $sql = "
        CREATE TABLE `{$db->prefix}simple_vocab_definitions` (
            `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
            `term_name` varchar(200) COLLATE utf8_unicode_ci NOT NULL,
            `term_definition` mediumtext COLLATE utf8_unicode_ci NOT NULL,
            PRIMARY KEY (`id`),
    ...
    public static function uninstall()
    {
        ...
        $db->query($sql);
        $sql = "DROP TABLE IF EXISTS `{$db->prefix}simple_vocab_definitions`;";
        $db->query($sql);
    }
    ...
    public static function adminNavigationMain($nav)
    {
        ...
        $nav['Simple Definitions'] = uri('simple-definitions');
        return $nav;
    }
